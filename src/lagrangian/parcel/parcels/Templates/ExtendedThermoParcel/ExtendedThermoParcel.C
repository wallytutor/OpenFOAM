...
void Foam::ExtendedThermoParcel<ParcelType>::setCellValues
(
...
template<class ParcelType>
template<class TrackCloudType>
void Foam::ExtendedThermoParcel<ParcelType>::cellValueSourceCorrection
(
    TrackCloudType& cloud,
    trackingData& td,
    const scalar dt
...
template<class ParcelType>
template<class TrackCloudType>
void Foam::ExtendedThermoParcel<ParcelType>::calcSurfaceValues
(
...
template<class ParcelType>
template<class TrackCloudType>
void Foam::ExtendedThermoParcel<ParcelType>::calc
(
    TrackCloudType& cloud,
    trackingData& td,
    const scalar dt
)
{
    ...
    // WWW: When no composition model is used (or a dummy single-phase
    // model is used), Cp_ never queries the solid component data with
    // the instantaneous temperature during heat transfer integration.
    // static const scalarField Y(1, 1);
    // Cp_ = cloud.composition().Cp(0, Y, td.pc(), T0);
    // WWW: maintain Cp evaluated at the newly attained parcel temperature
    this->Cp_ = this->solidCp(cloud, td.pc(), this->T_);
...
template<class ParcelType>
template<class TrackCloudType>
Foam::scalar Foam::ExtendedThermoParcel<ParcelType>::calcHeatTransfer
(
    TrackCloudType& cloud,
    trackingData& td,
    const scalar dt,
...
template<class ParcelType>
Foam::ExtendedThermoParcel<ParcelType>::ExtendedThermoParcel
(
...
#include "ExtendedThermoParcelIO.C"
