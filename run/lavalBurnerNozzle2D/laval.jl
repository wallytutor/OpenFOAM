...
# ╔═╡ 4310bb8a-33f4-42c8-9757-590d87b6b5d0
using CairoMakie

# ╔═╡ 4c2c80bf-cd40-474c-8ee1-27410d560653
using ModelingToolkit

# ╔═╡ 0c3a570a-04d6-4da9-b79f-0f6eed4030de
using NonlinearSolve

# ╔═╡ e99582c2-4fb5-4c9e-b5ac-52e881bc39ea
md"""
# Solution of *de Laval* nozzle geometry
"""

# ╔═╡ 20edbf8a-6a80-4254-82ff-3fd1deae6a7b
md"""
## Importing tools
"""

# ╔═╡ 507ce529-e95c-4bad-bc68-5098b8252ff3
md"""
## Problem implementation
"""

# ╔═╡ 1b7b2e01-30a7-40d8-a111-9d53f40cd85c
function getlavalsystem()
	pars = @parameters(begin
		rc , [description="Radius of nozzle neck"]
		yc , [description="Center of curvature of nozzle neck"]
		xv , [description="X-coordinate of cones interception point"]
		xa , [description="X-coordinate of start of compression"]
		ya , [description="Y-coordinate of start of compression"]
		xb , [description="X-coordinate of end of decompression"]
		yb , [description="Y-coordinate of end of decompression"]
	end)

	vars = @variables(begin
		ma  , [description="Slope of compression side"]
		mb  , [description="Slope of decompression side"]
		xc  , [description="Center of circle"]
		yv  , [description="Y-coordinate of cones interception point"]
		x1 , [description="X-coordinate of compression tangent"]
		y1 , [description="Y-coordinate of compression tangent"]
		x2 , [description="X-coordinate of decompression tangent"]
		y2 , [description="Y-coordinate of decompression tangent"]
	end)

	# Evaluation of intercept points from lines:
	fa = y1 + ma * (xv - x1)
	fb = y2 + mb * (xv - x2)

	# Distance from tangent points to intercept:
	da = (xv - x1)^2 + (fa - y1)^2
	db = (xv - x2)^2 + (fb - y2)^2

	eqs = [
		# Slopes of lines using tangent points.
		ma ~ (y1 - ya) / (x1 - xa)
		mb ~ (y2 - yb) / (x2 - xb)
		...
		# Matching slope with hemi-circle derivative:
		ma ~ (x1 - xc) / (rc^2 - (x1 - xc)^2)^(1//2)
		mb ~ (x2 - xc) / (rc^2 - (x2 - xc)^2)^(1//2)
	...
	@named laval = NonlinearSystem(eqs, vars, pars)
	return structural_simplify(laval)
end

# ╔═╡ 48190ed0-c909-4099-b2c5-5988183b349d
function solvelaval(;
		rc = 0.0060,
		yc = 0.0108,
		xv = 0.1081,
	...
	prob = NonlinearProblem(laval, guesses, pars; jac = true)
	sol = solve(prob)

	@info sol.retcode, sol.u

	return sol
end

# ╔═╡ a0c473db-eb80-4e17-8922-db6e20641993
function plotlavalsolution(sol; xticks=0.09:0.01:0.21, yticks=0.00:0.01:0.07)
	with_theme(theme_black()) do
		...
		let
			xs = LinRange(x1, x2, 100)
			ys = @. yc - (rc^2 - (xs - xc)^2)^(1/2)
			lines!(ax, xs, ys; color = :white)
		end

		lines!(ax, [xa, x1], [ya, y1]; color = :white)
		lines!(ax, [x2, xb], [y2, yb]; color = :white)

		scatter!(ax, xc, yc; color = :red)
		scatter!(ax, xa, ya; color = :red)
		scatter!(ax, xb, yb; color = :red)

		scatter!(ax, x1, y1; color = :red)
		scatter!(ax, x2, y2; color = :red)
		scatter!(ax, xv, yv; color = :red)

		vlines!(ax, xv; color = :white, linestyle = :dash)

		ax.title  = "de Laval nozzle solution"
		ax.xlabel = "Coordinate [m]"
		ax.ylabel = "Coordinate [m]"
		ax.xticks = xticks
		ax.yticks = yticks
...
# ╔═╡ 02223558-562c-481d-9659-8d9d0e9ad4f9
function extractpoints(sol)
	x = [
		0.0,          # Origin
		sol.ps[:xa],  # Start of compression
		sol[:x1],     # Start of neck
		sol[:x2],     # End of neck
		sol.ps[:xb],  # End of decompression
	]

	y = [
		0.0,          # 1 - Origin
		sol[:y1],     # 2 - Start of neck
		sol[:y2],     # 3 - End of neck
		sol.ps[:ya],  # 4 - Start of compression
		sol.ps[:yb],  # 5 - End of decompression
	]

	return x, y
end

# ╔═╡ 0d96d2cf-f542-4381-93f9-8e2c4a091884
function generatepointsgeo(sol; saveas = "laval-points.geo")
	x, y = extractpoints(sol)
...
# ╔═╡ 5d6962d0-ede9-4df4-9c0d-4b7a01b376f0
md"""
## Application scenario
"""

# ╔═╡ eda80fcc-cfc4-43fe-a239-028678e936c5
begin
	sol = solvelaval(;
		rc = 0.0060,
		yc = 0.0108,
	...
	generatepointsgeo(sol; saveas = "points.geo")
	plotlavalsolution(sol)
end
...
manifest_format = "2.0"
project_hash = "469c0fc471104052a9ec666940cf80f8a2252d24"
